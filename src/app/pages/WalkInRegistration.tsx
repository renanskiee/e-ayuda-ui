...
export default function WalkInRegistration() {
  ...
  // Update required documents when assistance type changes
  useEffect(() => {
    if (applicationData.assistanceType) {
      const docs = getRequiredDocuments(applicationData.assistanceType);
      setRequiredDocs(docs);

      // Initialize uploadedDocs state
  ...
  }, [applicationData.assistanceType]);
  ...
  const handleRegisterAndApply = (e: React.FormEvent) => {
    ...
    Object.values(uploadedDocs).forEach(file => {
      if (file) allDocuments.push(file);
    });
  ...
  const handleRequirementUpload = (docId: string, e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          <form onSubmit={handleRegisterAndApply} className="p-6 space-y-6">
            ...
            <div className="pt-6 border-t border-gray-200">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">
                Required Documents for {applicationData.assistanceType || 'Selected Assistance'}
              </h3>

              {!applicationData.assistanceType && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                  <p className="text-sm text-yellow-800">
                    Please select an assistance type above to see the required documents.
                  </p>
                </div>
              )}
                  ...
                  {requiredDocs.map((doc) => (
                    <div key={doc.id} className="border border-gray-300 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <input
                          type="file"
                          onChange={(e) => handleRequirementUpload(doc.id, e)}
                          className="hidden"
                        ...
                        <label
                          ...
                          className={`px-4 py-2 rounded-lg cursor-pointer transition-colors text-sm flex items-center gap-2 ${
                            doc.required
                      ...
                      {uploadedDocs[doc.id] && (
                        <div className="flex items-center justify-between bg-green-50 border border-green-200 p-2 rounded mt-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-700">{uploadedDocs[doc.id]?.name}</span>
                          </div>
                          <button
                            type="button"
