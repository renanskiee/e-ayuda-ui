...
export default function EvaluationQueue() {
  ...
  useEffect(() => {
    const loadApplications = () => {
      // Get applications for Solo Parent sector with status Received or Under Evaluation
      const allApps = applicationStore.getAll();
      const sectorApps = allApps.filter(app =>
        app.sector === 'Solo Parent' &&
  ...
  const handleSubmitEvaluation = () => {
    ...
    const currentUser = localStorage.getItem('username') || 'sector-sp-001';
    ...
    if (evaluationForm.decision === 'recommend') {
      ...
      const success = applicationStore.recommendApplication(
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-3xl font-semibold text-gray-800">Evaluation Queue - Solo Parent</h1>
        <p className="text-gray-500 mt-1">Review and evaluate pending solo parent applications</p>
      </div>
